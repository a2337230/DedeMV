...
function initWidget(config, apiPath = "/") {
	...
	document.body.insertAdjacentHTML("beforeend", `<div class="goGame live2d-tool" id="goGame">进入妹纸学院</div><div class="live2d-tool hide-live2d no-select" id="show_model"><div class="keys">隐藏</div></div>
		<div class="live2d-tool live2d-pio no-select" id="switch_live2d"><div class="keys">切换</div></div>
		<div id="switch_model"><!--class="live2d-tool switch-live2d no-select" <div class="keys">Switch</div>--></div>
		<div class="live2d-tool save-live2d no-select" id="save_pic"><div class="keys">拍照</div></div>
		`);
	...
	document.getElementById("goGame").addEventListener("click", () => {
		...
		game.className = "school"
		game.innerHTML = "<div class='goback'><button id='goback'>退出学院</button><h2>妹纸消消乐</h2><div class='music closeMusic'><img src='http://127.0.0.1/live2d/play.png'/></div></div><iframe class='game-wrap' src='http://192.168.105/gameschool/index.html' frameborder='0'></iframe>"
		wrap.appendChild(game)
		setTimeout(() => {
			var gameContainer = document.getElementsByClassName("school")[0]
			gameContainer.style.left = "0vw"
			// 监听关闭
			document.getElementById("goback").addEventListener("click", () => {
				var gameContainer = document.getElementsByClassName("school")[0]
				gameContainer.style.left = "-100vw"
				setTimeout(() => {
					var wrap = document.documentElement
					if ($('audio')[0]) {
						wrap.removeChild($('audio')[0])
					}
					wrap.removeChild(document.getElementsByClassName("school")[0])
					isAudio = false
				}, 500)
			})
			$(".music").click(() => {
				if (!isAudio) {
					musicPlay = document.createElement('audio')
					let num = parseInt(Math.random()*37,10)+1
					musicPlay.src = "http://127.0.0.1/live2d/music/" +num+".mp3"
					document.documentElement.appendChild(musicPlay)
					isAudio = true
				}
				let names = $(".music")[0].className
				musicPlay = $('audio')[0]
				if (names.indexOf('closeMusic') > -1) {
					$(".music")[0].classList.add('music-animate')
					musicPlay.play()
				} else {
					musicPlay.pause()
					$(".music")[0].classList.remove('music-animate')
				}
				musicPlay.addEventListener('ended', function () {  
					let num1 = parseInt(Math.random()*37,10)+1
					musicPlay.src = "http://127.0.0.1/live2d/music/" +num1+".mp3"
					musicPlay.play()
					// alert('over');
				}, false)
				$(".music")[0].classList.toggle('closeMusic')
			})
		}, 20)
	})
	...
	toggle.addEventListener("click", () => {
		"Hide" == getCookie("live2d") ? setTimeout(function() {
                document.body.clientWidth > 860 && ($(".hide-live2d").css("bottom", "0px"), $(".save-live2d, .switch-live2d, .live2d-pio, .live2d-tia").addClass("hide-live2d-tool")),
                $(".hide-live2d .keys").html("显示"), $(".goGame").css("bottom", "65px"),
		document.querySelector("#waifu-tool .fa-times").click(),
                setCookie("live2d", "Show", 7)
        },
        10) : setTimeout(function() {
                document.body.clientWidth > 860 && ($(".hide-live2d").css("bottom", "130px"), $(".save-live2d, .switch-live2d, .live2d-pio, .live2d-tia").removeClass("hide-live2d-tool")),
                $(".hide-live2d .keys").html("隐藏"), $(".goGame").css("bottom", "195px"),
		localStorage.removeItem("waifu-display"),
