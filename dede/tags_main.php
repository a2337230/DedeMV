...
else if($action == 'update')
{
    ...
    $query = "UPDATE `#@__tagindex` SET `count`='$count',`tagms`='$tagms' WHERE id='$tid' ";
    ...
    while($row = $dsql->GetArray())
    {
        ...
        foreach($keyarr as $keyword)
        {
            ...
            if($keyword != '' && strlen($keyword)<13 )
            {
                ...
                {
                    $query = " INSERT INTO `#@__tagindex`(`tag`,`tagms`,`count`,`total`,`weekcc`,`monthcc`,`weekup`,`monthup`,`addtime`) VALUES('$keyword','$tagms','0','1','0','0','$timestamp','$timestamp','$timestamp');";
                    $dsql->ExecuteNoneQuery($query);
                    $tid = $dsql->GetLastID();
                }
